...
void slave_setup() {
    Serial.begin(9600);
    _SPI.onSelect(onSelect);
    _SPI.setClockSpeed(1, MHZ);
    _SPI.setDataMode(SPI_MODE0);
    _SPI.begin(SPI_MODE_SLAVE, _CS);
...
test(01_SPI_Master_Slave_Slave_Transfer)
{
    ...
    while (1) {
        ...
        transfer_state = 0;
        _SPI.transfer(tx_buffer, rx_buffer, sizeof(rx_buffer), onTransferFinished);
        ...
        int bytesReceived = _SPI.available();
        if (bytesReceived > 0) {
